...
def parse_args():
    ...
    parser.add_argument("--n_samples", default=40, type=int)
    parser.add_argument("--logprobs", default=1, type=int)
    parser.add_argument("--use_mini_n", default=False, action='store_true')
    parser.add_argument("--mini_n_samples", default=40, type=int, help='value of n for mini code generation sampling (when token rate is limited)')
...
if __name__ == "__main__":
    ...
    if args.greedy:
        filename = f'{args.output_dir}/{args.dt_name}_{mtype}_s{args.start}_e{args.end}_{dt_string}_seed{args.seed}_entropy_low{args.entropy_threshold_low}_entropy_high{args.entropy_threshold_high}_bestof{args.mini_n_samples}.jsonl'
        assert not args.temperature, "In greedy decoding, temperature should be 0.0"
    else:
        if args.n_samples > 1:
            filename = f'{args.output_dir}/{args.dt_name}_sc_{mtype}_tp{args.temperature}_topp{args.top_p}_s{args.start}_e{args.end}_{dt_string}_seed{args.seed}_entropy_low{args.entropy_threshold_low}_entropy_high{args.entropy_threshold_high}_bestof{args.mini_n_samples}.jsonl'
        else:
            filename = f'{args.output_dir}/{args.dt_name}_vanilla_{mtype}_tp{args.temperature}_topp{args.top_p}_s{args.start}_e{args.end}_{dt_string}_seed{args.seed}_entropy_low{args.entropy_threshold_low}_entropy_high{args.entropy_threshold_high}_bestof{args.mini_n_samples}.jsonl'
    if args.reverse:
    ...
    for batch_idx in tqdm(range((len(inputs) + args.batch_size - 1) // args.batch_size)):
        ...
        if len(batch) == 1:
            # Replace Counter with tracking best answer and probability
            best_prob = float('-inf')
            best_ans = None
            
            for gen_result in raw_results['choices']:
                code = gen_result['text']
                # Get the total log probability for this generation
                logprob = sum(gen_result['logprobs']['token_logprobs'])
                logprob = logprob / len(gen_result['logprobs']['tokens'])
                
                ans = safe_execute(code)
                ans = floatify_ans(ans)
                
                if ans is not None:
                    # Update best answer if this has higher probability
