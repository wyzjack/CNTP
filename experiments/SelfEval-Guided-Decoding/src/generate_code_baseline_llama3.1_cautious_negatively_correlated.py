...
if __name__ == "__main__":
    ...
    if args.greedy:
        filename = f'{args.output_dir}/{args.dt_name}_{mtype}_s{args.start}_e{args.end}_{dt_string}_seed{args.seed}_entropy_low{args.entropy_threshold_low}_entropy_high{args.entropy_threshold_high}_trials{args.max_trials}_cautious_perplexity.jsonl'
        assert not args.temperature, "In greedy decoding, temperature should be 0.0"
    else:
        if args.n_samples > 1:
            filename = f'{args.output_dir}/{args.dt_name}_sc_{mtype}_tp{args.temperature}_topp{args.top_p}_s{args.start}_e{args.end}_{dt_string}_seed{args.seed}_entropy_low{args.entropy_threshold_low}_entropy_high{args.entropy_threshold_high}_trials{args.max_trials}_cautious_perplexity_negatively_correlated.jsonl'
        else:
            filename = f'{args.output_dir}/{args.dt_name}_vanilla_{mtype}_tp{args.temperature}_topp{args.top_p}_s{args.start}_e{args.end}_{dt_string}_seed{args.seed}_entropy_low{args.entropy_threshold_low}_entropy_high{args.entropy_threshold_high}_trials{args.max_trials}_cautious_perplexity_negatively_correlated.jsonl'
    if args.reverse:
    ...
    generation_config = GenerationConfig(
        ...
        cntp_negatively_correlated=True
    )
