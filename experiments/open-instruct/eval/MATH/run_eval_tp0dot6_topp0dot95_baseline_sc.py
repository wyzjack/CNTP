...
def main(args):
    # Add HF login at the start of main if token is provided
    if args.hf_token:
    ...
    if args.model_name_or_path:
        ...
        if args.use_vllm:
            ...
            prompt_to_outputs = {
                g.prompt: [output.text for output in g.outputs] for g in generations
            }
            outputs = [prompt_to_outputs.get(prompt, [""] * 40) for prompt in prompts]
        else:
            model = load_hf_lm(
                model_name_or_path=args.model_name_or_path,
                revision=args.hf_revision,
            ...
            outputs = generate_completions_original(
                model=model,
                tokenizer=tokenizer,
                prompts=prompts,
                max_new_tokens=1024,
            ...
            # Fix: Reshape flat list into list of lists with 40 entries per prompt
            outputs = [outputs[i*40:(i+1)*40] for i in range(len(prompts))]
    else:
    ...
    for output_group in outputs:
        # Process 40 samples per question
        all_answers = []
        for output in output_group:  # Now handling multiple outputs per question
            unnormalized = get_unnormalized_answer(output)
            normalized = normalize_final_answer(unnormalized)
    ...
    predictions = [{
        ...
        "model_outputs": output_group,
        "all_unnormalized": [get_unnormalized_answer(o) for o in output_group],
        "all_predictions": [pred for pred in predictions if pred != ""],
        "prediction": pred
    } for example, output_group, pred in zip(test_data, outputs, predictions)]
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        help="Hugging Face token for authentication. Required for private model access and uploading results."
    )
