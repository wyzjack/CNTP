...
def create_prompt_with_llama3_1_chat_format(messages, tokenizer, bos="<|begin_of_text|>", eos="<|eot_id|>", add_bos=True):
    formatted_text = bos if add_bos else ""
    for message in messages:
        role = message["role"]
        if role == "user":
            ...
            # System message at beginning gets special formatting
            if len(formatted_text) == len(bos) or not add_bos:  # Handle bos presence
                formatted_text += f"<|start_header_id|>system<|end_header_id|>\n{message['content']}{eos}\n"
            else:
                raise ValueError("Llama3.1 chat template requires system message to be at the beginning of the conversation")
        else:
            raise ValueError(f"Llama3.1 chat template only supports 'system', 'user' and 'assistant' roles. Invalid role: {role}.")
    return formatted_text

def create_prompt_with_deepseek_r1_distill_qwen_1_dot_5B_format(messages, tokenizer, bos="<｜begin▁of▁sentence｜>", eos="<｜end▁of▁sentence｜>", add_bos=True):
    """Format messages for DeepSeek-R1-Distill-Qwen-1.5B using Qwen-style chat format"""
    formatted_text = bos if add_bos else ""
    ...
    for i, message in enumerate(messages):
        ...
        if role == "system":
            if i != 0:
                raise ValueError("System message must be at the beginning of the conversation")
            ...
            raise ValueError(f"DeepSeek-R1-Distill-Qwen-1.5B chat template only supports 'system', 'user', and 'assistant' roles. Invalid role: {role}.")
