...
@torch.no_grad()
def generate_completions(model, tokenizer_1, prompts, batch_size=1, stop_id_sequences=None, add_special_tokens=True, disable_tqdm=False, entropy_threshold_low=0.01, entropy_threshold_high=1.5, max_trials=10, **generation_kwargs):
    ...
    for i in range(0, len(prompts), batch_size):
        ...
        tokenized_prompts = tokenizer_1(batch_prompts, padding="longest", return_tensors="pt", add_special_tokens=add_special_tokens)
        ...
        try:
                ...
                for output_idx in range(batch_outputs.shape[0]):
                    for token_idx in range(batch_input_ids.shape[1], batch_outputs.shape[1]):
                        if any(batch_outputs[output_idx, token_idx: token_idx+len(stop_sequence)].tolist() == stop_sequence for stop_sequence in stop_id_sequences):
                            batch_outputs[output_idx, token_idx:] = tokenizer_1.pad_token_id
                            break
            ...
            batch_outputs = tokenizer_1.batch_decode(batch_outputs, skip_special_tokens=True)
            batch_prompts = tokenizer_1.batch_decode(batch_input_ids, skip_special_tokens=True)
...
@torch.no_grad()
def generate_completions_original(model, tokenizer, prompts, batch_size=1, stop_id_sequences=None, add_special_tokens=True, disable_tqdm=False, beam_size=5, **generation_kwargs):
...
@torch.no_grad()
def generate_completions_beam(model, tokenizer, prompts, batch_size=1, stop_id_sequences=None, add_special_tokens=True, disable_tqdm=False, **generation_kwargs):
